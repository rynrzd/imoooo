...
const WHY_GLOWS = ["var(--nireo-glow-a)", "var(--nireo-glow-b)", "var(--nireo-glow-c)", "oklch(0.82 0.09 60)"];
    ...
    <>
        ...
        <div className="mx-auto max-w-3xl px-4 pt-20 pb-14 text-center sm:px-6 sm:pt-24">
          <Reveal>
            ...
            <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
              <a href="#recrutement" className={cn(buttonVariants({ size: "lg" }), "nireo-glow h-11 px-6")}>
                Travailler avec nous <ArrowRight className="size-4" />
              </a>
              <Link href="/" className={cn(buttonVariants({ variant: "outline", size: "lg" }), "nireo-glass-soft h-11 px-6 text-foreground")}>
          ...
          {p.vision || p.mission ? (
            <div className="mx-auto mt-10 grid max-w-4xl gap-4 md:grid-cols-2">
              {p.vision ? (
                <Reveal>
                  <div className="nireo-glass h-full rounded-2xl p-6">
                    <p className="flex items-center gap-2 text-[11px] font-semibold tracking-widest text-primary uppercase"><Compass className="size-4" /> Vision</p>
              ...
              {p.mission ? (
                <Reveal delay={80}>
                  <div className="nireo-glass h-full rounded-2xl p-6">
                    <p className="flex items-center gap-2 text-[11px] font-semibold tracking-widest text-primary uppercase"><Rocket className="size-4" /> Mission</p>
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            {p.partners.map((partner, i) => (
              <Reveal key={partner.name + i} delay={(i % 6) * 40}>
                {partner.url ? (
                  <a href={partner.url} target="_blank" rel="noopener noreferrer" className="nireo-glass-soft inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium text-foreground transition-colors hover:text-primary">
                    <Handshake className="size-4 text-primary" /> {partner.name} <ArrowUpRight className="size-3.5 text-muted-foreground" />
                  </a>
                ) : (
                  <span className="nireo-glass-soft inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium text-foreground"><Handshake className="size-4 text-primary" /> {partner.name}</span>
                )}
              </Reveal>
            ))}
          </div>
            ...
            {p.press.map((article, i) => (
              <a key={article.title + i} href={article.url || "#"} target={article.url ? "_blank" : undefined} rel="noopener noreferrer" className="flex items-center gap-4 p-5 transition-colors hover:bg-muted">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">{[article.outlet, article.date].filter(Boolean).join(" · ")}</p>
                </div>
      ...
      {p.recruitment.intro || p.recruitment.reasons.length > 0 || p.recruitment.lookingFor.length > 0 ? (
        <Band id="recrutement" className="relative overflow-hidden">
          <div aria-hidden className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(60%_120%_at_50%_0%,var(--nireo-glow-b),transparent_60%)] opacity-15" />
          ...
          {p.recruitment.lookingFor.length > 0 ? (
            <Reveal className="mt-8">
              <div className="flex flex-wrap items-center justify-center gap-2.5">
                <span className="text-sm text-muted-foreground">Nous recherchons&nbsp;:</span>
                {p.recruitment.lookingFor.map((l, i) => (
                  <span key={l.label + i} className="nireo-glass-soft inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-sm font-medium text-foreground">
                    <Briefcase className="size-3.5 text-primary" /> {l.label}
                  </span>
                ))}
              </div>
            </Reveal>
          ...
          {p.recruitment.ctaEmail ? (
            <div className="mt-10 flex justify-center">
              <a href={`mailto:${p.recruitment.ctaEmail}`} className={cn(buttonVariants({ size: "lg" }), "nireo-glow h-11 px-6")}>
      ...
      {p.faq.length > 0 ? (
        <Band>
          ...
          <div className="mx-auto mt-10 max-w-3xl space-y-3">
            {p.faq.map((item, i) => (
              <details key={item.question + i} className="nireo-glass group/faq rounded-2xl px-5 py-4 [&_summary]:list-none">
      ...
      <Band id="contact" className="relative overflow-hidden">
        <div aria-hidden className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(60%_120%_at_50%_0%,var(--nireo-glow-a),transparent_60%)] opacity-20" />
        ...
        <div className="mx-auto mt-12 grid max-w-4xl gap-4 sm:grid-cols-2">
          ...
          {p.address || location ? (
            <div className="nireo-glass flex items-center gap-4 rounded-2xl p-5">
              ...
              <div><p className="text-xs text-muted-foreground">Adresse</p><p className="font-medium text-foreground">{p.address || location}</p></div>
            </div>
          ) : null}
          {p.hours ? (
            <div className="nireo-glass flex items-center gap-4 rounded-2xl p-5">
        ...
        {p.social.length > 0 || p.website ? (
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
            {p.website ? (
              <a href={p.website} target="_blank" rel="noopener noreferrer" className="nireo-glass-soft inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium text-foreground transition-colors hover:text-primary">
                Site web <ArrowUpRight className="size-3.5 text-muted-foreground" />
              </a>
            ) : null}
            {p.social.map((s, i) => (
              <a key={s.platform + i} href={s.url} target="_blank" rel="noopener noreferrer" className="nireo-glass-soft inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium text-foreground transition-colors hover:text-primary">
                {s.platform} <ArrowUpRight className="size-3.5 text-muted-foreground" />
              </a>
            ))}
          </div>
        ) : null}
