 ...
 */
export function NireoMark({ className, flat = false }: { className?: string; flat?: boolean }) {
  return (
    <span
      className={cn(
        "relative grid size-9 shrink-0 place-items-center overflow-hidden rounded-[0.7rem]",
        flat
          ? // Version PLATE (landing claire) : encre sur bleu très pâle,
            // aucun dégradé, aucun halo, aucune ombre portée.
            "rounded-[0.45rem] bg-[var(--land-blue-pale,#e3e9f5)] text-[var(--land-blue,#3158d4)]"
          : [
    ...
    >
      {/* Halo interne — réservé à l'univers sombre. */}
      {flat ? null : (
        <span
          aria-hidden
      ...
      <svg viewBox="0 0 24 24" className="relative size-5" fill="none" aria-hidden>
        ...
        <path
          ...
          stroke={flat ? "currentColor" : "url(#nireo-mark)"}
          strokeWidth="2.4"
          strokeLinecap="round"
    ...
    >
      ...
      <span
        className={cn(
          ...
          onDark ? "text-[var(--land-paper,#fcfbf8)]" : "text-foreground",
